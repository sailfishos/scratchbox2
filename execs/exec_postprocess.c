...
int machine_to_preloader_platform(uint16_t machine, char* preloader) {
	if (machine == HOST_ELF_MACHINE_32)
		return 1;
#ifdef HOST_ELF_MACHINE_64
	if (machine == HOST_ELF_MACHINE_64)
		return 1;
#endif
	return -1;
}
...
#define str_not_empty(s) ((s) && *(s))
			char *cp;
			machine_to_preloader_platform(info->machine, (char *)native_app_ld_preload_prefix);
			machine_to_preloader_platform(info->machine, (char *)native_app_ld_preload_suffix);
			if (libpath) {
